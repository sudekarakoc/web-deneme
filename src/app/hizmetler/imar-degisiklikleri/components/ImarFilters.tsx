...
interface FiltersProps {
  ...
  ilceFilter: string;
  setIlceFilter: (value: string) => void;
  durumFilter: string;
  setDurumFilter: (value: string) => void;
  planTuruFilter: string;
  setPlanTuruFilter: (value: string) => void;
}

export default function ImarFilters({
  aramaMetni,
  setAramaMetni,
  ilceFilter,
  setIlceFilter,
  ...
  return (
    <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm mb-6 flex flex-col gap-4">
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Mahalle, ada, parsel veya karar no ile arayın..."
          value={aramaMetni}
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex-1 relative">
          ...
          >
            ...
            <option value="Tekirdağ Merkez">Tekirdağ Merkez</option>
          </select>
          <svg
            className="w-5 h-5 absolute right-4 top-3.5 text-gray-400 pointer-events-none"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <div className="flex-1 relative">
          <select
            aria-label="Plan türü seçin"
            value={planTuruFilter}
            onChange={(e) => setPlanTuruFilter(e.target.value)}
            className="w-full pl-4 pr-10 py-3 rounded-xl border border-gray-200 bg-gray-50/50 text-[15px] text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#009FE3]/30 focus:border-[#009FE3] transition-all appearance-none cursor-pointer"
          >
            <option value="">Tüm Plan Türleri</option>
            <option value="1/1000 UİP">1/1000 Uygulama İmar Planı</option>
            <option value="1/5000 NİP">1/5000 Nazım İmar Planı</option>
            <option value="Revizyon">Revizyon</option>
            <option value="İlave">İlave İmar Planı</option>
          </select>
          <svg
            className="w-5 h-5 absolute right-4 top-3.5 text-gray-400 pointer-events-none"
            fill="none"
