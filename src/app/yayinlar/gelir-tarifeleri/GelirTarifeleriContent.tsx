...
import { Download, FileText, FileSpreadsheet, Building2, Calendar, FileDown, Search } from "lucide-react";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import { SITE_DATA } from "@/lib/data";
import Sidebar from "@/components/Sidebar";
import { gelirTarifeleri } from "./data";

export default function GelirTarifeleriContent() {
  const kategori = "yayinlar";
  const slug = "gelir-tarifeleri";
  const categoryData = SITE_DATA[kategori];

  const [activeYear, setActiveYear] = useState<number>(gelirTarifeleri[0]?.year || 2026);
  ...
  const activeData = gelirTarifeleri.find(y => y.year === activeYear);
      ...
      <div className="w-full bg-[#EAF4E2] pt-[150px] pb-5 px-6 lg:px-8 border-b border-[#73B646]/20">
        <div className="max-w-7xl mx-auto flex flex-col gap-2">
          <div className="flex flex-wrap items-center gap-1.5 text-sm text-gray-600 font-medium tracking-wide">
            ...
            <span className="text-gray-400">/</span>
            <span className="text-[#009FE3] font-bold">Gelir Tarifeleri</span>
          </div>
        </div>
      </div>
          ...
          <div ref={headerRef} className="mb-10 flex flex-col sm:flex-row sm:items-end justify-between gap-6">
            <div className="max-w-2xl">
              ...
              <p className="text-[15px] sm:text-[16px] text-gray-500 mt-3 leading-relaxed">
                Tekirdağ Büyükşehir Belediyesi daire başkanlıklarına ait ücrete tabi işler ve gelir tarifelerine ait güncel ve geçmiş yılların belgelerine buradan ulaşabilirsiniz.
              </p>
            </div>
            ...
            <div className="relative w-full sm:w-72 shrink-0">
              ...
              <input 
                ...
                placeholder="Daire başkanlığı ara..." 
          ...
          {filteredFiles.length > 0 ? (
            <div ref={gridRef} className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
              {filteredFiles.map((file, index) => {
                const isExcel = file.url.toLowerCase().endsWith('.xlsx') || file.url.toLowerCase().endsWith('.xls');
                return (
                  <a
                    ...
                    <div className="flex items-start gap-4 z-10 h-full flex-col justify-between">
                      <div className="flex items-start gap-3 w-full">
                        <div className={`w-10 h-10 shrink-0 rounded-xl flex items-center justify-center
                          ${isExcel ? "bg-green-50 text-green-600 group-hover:bg-green-600 group-hover:text-white" : "bg-red-50 text-red-500 group-hover:bg-red-500 group-hover:text-white"} transition-colors`}>
                      ...
                      <div className="flex items-center justify-between w-full mt-4 pt-4 border-t border-gray-50 group-hover:border-gray-100 transition-colors">
                        <span className={`text-[11px] font-bold tracking-wider uppercase ${isExcel ? "text-green-600" : "text-red-500"}`}>
                          {isExcel ? 'EXCEL DOSYASI' : 'PDF DOSYASI'}
                        </span>
